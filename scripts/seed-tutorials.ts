/**
 * Seed script to populate the database with tech tutorials
 *
 * Usage:
 * 1. Make sure you have your Firebase service account JSON
 * 2. Run: npx ts-node --project tsconfig.seed.json scripts/seed-tutorials.ts
 *
...
// Try to load service account from file first, then fall back to env vars
...
if (fs.existsSync(serviceAccountPath)) {
  ...
  if (!serviceAccount.projectId || !serviceAccount.clientEmail || !serviceAccount.privateKey) {
    ...
    console.log('\n1. Create a service-account.json file in the project root:');
    console.log('   - Go to Firebase Console > Project Settings > Service Accounts');
    console.log('   - Click "Generate new private key"');
    console.log('   - Save the file as "service-account.json" in the project root');
...
// Tutorial content
const tutorials: Omit<TutorialData, 'slug' | 'readingTime' | 'createdAt' | 'updatedAt' | 'publishedAt' | 'views' | 'likes'>[] = [
  {
...
async function seedTutorials() {
  ...
  for (const tutorial of tutorials) {
    ...
    const existingQuery = await db.collection('tutorials').where('slug', '==', slug).get();

    if (!existingQuery.empty) {
      console.log(`⏭️  Skipping "${tutorial.title}" - already exists`);
      skipCount++;
      continue;
    }
    ...
    const tutorialData: TutorialData = {
      ...tutorial,
      slug,
      readingTime: calculateReadingTime(tutorial.content),
      createdAt: now,
      updatedAt: now,
      publishedAt: tutorial.published ? now : null,
      views: 0,
      likes: 0,
    };
    ...
    try {
      ...
      console.log(`✅ Created: "${tutorial.title}" (${docRef.id})`);
      successCount++;
    } catch (error) {
      console.error(`❌ Failed to create "${tutorial.title}":`, error);
    }
  }
